    ...
    <div class="table-responsive">
        <table class="table table-striped table-bordered table-hover">
            ...
            <?php if(!empty($list)): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$com): $mod = ($i % 2 );++$i;?><tr>
                        ...
                        <td>
                            <?php if($com['status'] == 2): ?><a title="设为通过" class="ajax-get"   href="<?php echo _U('company/resume',array('id'=>$com['id']));?>">审核通过</a>
                                <a title="设为不通过" class="verifyNot" data-id="<?php echo ($com['id']); ?>" data-uid="<?php echo ($com["uid"]); ?>"  data-url="<?php echo _U('company/verifyNot');?>" href="javascript:;">审核不通过</a><?php endif; ?>
                        </td>
